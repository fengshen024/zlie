...
class Baseline(pl.LightningModule, ABC):
    ...
    def __init__(self, config):
        ...
        self.Cr = nn.Linear(hidden_size, config.num_rels * 3)
        self.hr_rev = nn.Linear(config.num_rels * 3, hidden_size)
        self.tr_rev = nn.Linear(config.num_rels * 3, hidden_size)
        ...
        torch.nn.init.orthogonal_(self.e.weight, gain=1)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        tail_logits = table_logist[:, self.config.num_rels:self.config.num_rels*2, :, :]
        span_logits = table_logist[:, self.config.num_rels*2:, :, :].mean(dim=1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("tail_loss", tail_loss, prog_bar=True)
        self.log("span_loss", span_loss, prog_bar=True)
